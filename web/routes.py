...
def home():
    ...
    #     model = ToxicModel()
    #     model.state_dict(torch.load("ml/to_upload/Toxificator-512-model.pth", map_location=torch.device('cpu')))
    #     model.eval()
    ...
    if request.method == "POST":
        ...
        model.state_dict(
            torch.load(
                "ml/to_upload/Nation_model.pth", map_location=torch.device("cpu")
            )
        )
        model.eval()
        user_input = request.form.get("user_input_text")
        with open("ml/models/vectorizer_for_Nation.json") as file:
